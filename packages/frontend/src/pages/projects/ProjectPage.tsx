      ...
      {project && summary && (
        <Container maxWidth={false} sx={{ px: { xs: 2, md: 3 } }}>
          ...
          <Grid container spacing={{ xs: 1.5, sm: 3 }} sx={{ mb: 4 }}>
            <Grid size={{ xs: 6, md: 3 }}>
              <SummaryCard title="Бюджет" value={summary.budget != null ? formatCurrency(summary.budget) : "Не задан"} icon={<AccountBalanceIcon />} color="info.main" />
            </Grid>
            <Grid size={{ xs: 6, md: 3 }}>
              <SummaryCard title="Потрачено" value={formatCurrency(summary.totalSpent)} icon={<AttachMoneyIcon />} color="warning.main" />
            </Grid>
            <Grid size={{ xs: 6, md: 3 }}>
              <SummaryCard title="Запланировано" value={formatCurrency(summary.plannedTotal)} icon={<PlaylistAddCheckIcon />} color="info.main" />
            </Grid>
            <Grid size={{ xs: 6, md: 3 }}>
              <SummaryCard title="Остаток" value={summary.remaining != null ? formatCurrency(summary.remaining) : "Н/Д"} icon={<SavingsIcon />} color={summary.remaining != null && summary.remaining < 0 ? "error.main" : "success.main"} />
            </Grid>
          </Grid>
